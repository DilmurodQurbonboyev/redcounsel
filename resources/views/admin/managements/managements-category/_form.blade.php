<div class="form-group">
    ...
    <select class="custom-select rounded-0 select2" name="parent_id" id="parent_id">
        ...
        @foreach ($managementCategories as $management)
            <option
                value="{{ $management->id }}" {{ old('parent_id', $managementCategory->parent_id) == $management->id ? 'selected' : '' }}>{{ $management->title }}</option>
        @endforeach
    </select>
</div>
        ...
        <div class="tab-pane fade show" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}">
            <div class="form-group">
                ...
                <input type="text" name="title_{{ $lang }}"
                       ...
                       id="title_{{ $lang }}"
                       value="{{ old('title_'.$lang, $managementCategory->translate($lang)->title ?? '') }}">
                @error('title_'.$lang)
...
<div class="form-group">
    ...
    <div class="input-group">
        <input id="thumbnail_image" class="form-control" type="text" name="image"
               value="{{ old('image', $managementCategory->image ?? '') }}">
        <span class="input-group-btn">
            <a id="image" data-input="thumbnail_image" data-preview="holder"
...
<div class="form-group">
    ...
    <select class="custom-select rounded-0 select2" name="status" id="status">
        <option
            value="2" {{ old('status', $managementCategory->status) == "2" ? 'selected' : '' }}>{{tr('Active')}}</option>
        <option
            value="1" {{ old('status', $managementCategory->status) == "1" ? 'selected' : '' }}>{{tr('Inactive')}}</option>
    </select>
</div>
