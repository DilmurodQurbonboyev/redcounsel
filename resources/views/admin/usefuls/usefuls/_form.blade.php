<div class="form-group">
    ...
    <select class="custom-select rounded-0 select2" name="lists_category_id" id="lists_category_id">
        ...
        <option value="{{ $usefulCategory->id }}" {{ old('lists_category_id', $usefuls->lists_category_id) ==
            $usefulCategory->id ? 'selected' : '' }}>{{ $usefulCategory->title }}</option>
        @endforeach
    </select>
</div>
    ...
    <div class="tab-pane fade show" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}">
        <div class="form-group">
            ...
            <input type="text" name="title_{{ $lang }}" class="form-control @error('title_'.$lang) is-invalid @enderror"
                id="title_{{ $lang }}" value="{{ old('title_'.$lang, $usefuls->translate($lang)->title ?? '') }}">
            @error('title_'.$lang)
            <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        <x-admin.form.description name="description_{{ $lang }}"
            value="{!! old('description_'.$lang, $usefuls->translate($lang)->description ?? '') !!}" />
    </div>
    @endforeach
</div>
...
<div class="row">
    <div class="form-group col-6">
        ...
        <input type="text" name="link" class="form-control" id="link" value="{{ old('link', $usefuls->link ?? '') }}">
        @error('link')
        <div class="text-danger">{{ $message }}</div>
        @enderror
    </div>
    <div class="form-group col-6">
        ...
        <select name="link_type" class="custom-select rounded-0 select2" id="link_type">
            <option value="1" {{ old('status', $usefuls->link_type) == '1' ? 'selected' : '' }}>{{ tr('Inactive') }}
            </option>
            <option value="2" {{ old('status', $usefuls->link_type) == '2' ? 'selected' : '' }}>{{ tr('Local opens in
                this window') }}</option>
            <option value="3" {{ old('status', $usefuls->link_type) == '3' ? 'selected' : '' }}>{{ tr('Local opens in a
                new window') }}</option>
            <option value="4" {{ old('status', $usefuls->link_type) == '4' ? 'selected' : '' }}>{{ tr('Global opens in
                this window') }}</option>
            <option value="5" {{ old('status', $usefuls->link_type) == '5' ? 'selected' : '' }}>{{ tr('Global opens in a
                new window') }}</option>
        </select>
    </div>
</div>
...
<div class="form-group">
    ...
    <select class="custom-select rounded-0 select2" name="parent_id" id="parent_id">
        <option value="2" {{ old('status', $usefuls->parent_id) == '2' ? 'selected' : '' }}>{{ tr('Yes') }}</option>
        <option value="1" {{ old('status', $usefuls->parent_id) == '1' ? 'selected' : '' }}>{{ tr('No') }}</option>
    </select>
</div>
<div class="form-group">
    <label for="status">{{ tr('Status') }}</label>
    <select class="custom-select rounded-0 select2" name="status" id="status">
        <option value="2" {{ old('status', $usefuls->status) == '2' ? 'selected' : '' }}>{{tr('Active')}}</option>
        <option value="1" {{ old('status', $usefuls->status) == '1' ? 'selected' : '' }}>{{tr('Inactive')}}</option>
    </select>
</div>
