<div class="form-group">
    ...
    <select class="custom-select rounded-0 select2" name="parent_id" id="parent_id">
        ...
        @foreach ($videosCategories as $video)
            <option
                value="{{ $video->id }}" {{ old('parent_id', $videosCategory->parent_id) == $video->id ? 'selected' : '' }}>
                {{ $video->title }}
            </option>
        @endforeach
    </select>
        ...
        <div class="tab-pane fade show" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}">
            <div class="form-group">
                ...
                <input type="text" name="title_{{ $lang }}"
                       ...
                       id="title_{{ $lang }}"
                       value="{{ old('title_'.$lang, $videosCategory->translate($lang)->title ?? '') }}">
                @error('title_'.$lang)
...
<div class="form-group">
    ...
    <div class="input-group">
        <input id="thumbnail_image" class="form-control" type="text" name="image"
               value="{{ old('image', $videosCategory->image ?? '') }}">
...
<div class="form-group">
    ...
    <select class="custom-select rounded-0 select2" name="status" id="status">
        <option value="2" {{ old('status', $videosCategory->status) == '2' ? 'selected' : '' }}>{{tr('Active')}}</option>
        <option value="1" {{ old('status', $videosCategory->status) == '1' ? 'selected' : '' }}>{{tr('Inactive')}}
        </option>
    </select>
</div>
