    ...
    <select class="custom-select rounded-0 select2 @error('lists_category_id') is-invalid @enderror"
        ...
        @foreach ($newsCategories as $newsCategory)
            <option value="{{ $newsCategory->id }}"
                {{ old('lists_category_id', $news->lists_category_id) == $newsCategory->id ? 'selected' : '' }}>
        ...
        <div class="tab-pane fade show" id="{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}">
            <div class="form-group">
                ...
                <input type="text" name="title_{{ $lang }}"
                    ...
                    value="{{ old('title_' . $lang, $news->translate($lang)->title ?? '') }}">
                @error('title_' . $lang)
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            <x-admin.form.description name="description_{{ $lang }}" value="{!! old('description_' . $lang, $news->translate($lang)->description ?? '') !!}" />
            <x-admin.form.content name="content_{{ $lang }}" value="{!! old('content_' . $lang, $news->translate($lang)->content ?? '') !!}" />
            {{--  <div class="form-group">
...
{{--  <div class="form-group">
    ...
    <select class="custom-select rounded-0 select2" name="pdf_type" id="pdf_type">
        <option value="2" {{ old('pdf_type', $news->pdf_type == '2' ? 'selected' : '') }}>{{ tr('Download') }}
        </option>
        <option value="1" {{ old('pdf_type', $news->pdf_type == '1' ? 'selected' : '') }}>{{ tr('Show') }}
        </option>
    </select>
</div>  --}}
<div class="form-group">
    ...
    <div class="input-group">
        <input value="{{ old('anons_image', $news->anons_image) }}" id="thumbnail_anons_image" class="form-control"
...
<div class="form-group">
    ...
    <input type="number" name="order" class="form-control" id="order"
        value="{{ old('order', $news->order ?? '50') }}">
</div>
<div class="form-group">
    <label for="date" class="@error('date') text-danger @enderror">{{ tr('Date') }}</label>
    <input type="text" class="form-control date @error('date') is-invalid @enderror" name="date"
...
{{--  <div class="form-group">
    ...
    <select class="custom-select rounded-0 select2" name="show_on_slider" id="show_on_slider">
        <option value="1" {{ old('show_on_slider', $news->show_on_slider) == '1' ? 'selected' : '' }}>
            {{ tr('No') }}</option>
        <option value="2" {{ old('show_on_slider', $news->show_on_slider) == '2' ? 'selected' : '' }}>
            {{ tr('Yes') }}</option>
    </select>
</div>  --}}
<div class="form-group">
    ...
    <select class="custom-select rounded-0 select2" name="status" id="status">
        <option value="2" {{ old('status', $news->status) == '2' ? 'selected' : '' }}>{{ tr('Active') }}
        </option>
        <option value="1" {{ old('status', $news->status) == '1' ? 'selected' : '' }}>{{ tr('Inactive') }}
        </option>
    </select>
</div>
