...
@endsection
@section('header')
    <x-admin.breadcrumb-show :show="$pages->title ?? '' " :breadcrumb="'pages/show'" :id="$pages->id"/>
@endsection
@section('content')
    <div class="p-3 d-flex justify-content-end" style="grid-gap: 2px">
        ...
        <a class="cards btn btn-success" href="{{ route('pages.create') }}">
            <i class="fa fa-plus"></i>
        </a>
        <a class="cards btn btn-info text-white" href="{{ route('pages.index') }}">
            <i class="fa fa-list"></i>
        </a>
        <a class="cards btn btn-primary" href="{{ route('pages.edit', $pages->id) }}">
                            ...
                            <table class="table  table-striped table-hover">
                                ...
                                <tr>
                                    ...
                                    <th>
                                        ...
                                        @else
                                            <a data-fancybox="single" href="{{ $pages->anons_image }}">
                                                <div class="foto-in">
                                                    <img width="300px" src="{{ $pages->anons_image }}"/>
                                                </div>
                                            </a>
                                        @endif
                                    </th>
                                ...
                                <tr>
                                    ...
                                    <th scope="row">
                                        @if($pages->pdf)
                                            ...
                                            <a href="{{ $pages->pdf }}" class="text-dark" target="__blank">
                                                {{ $pages->translate($lang)->pdf ?? '' }}
                                            </a>
                                        @endif
                                    </th>
                                </tr>
                                ...
                                <tr>
                                    <td>{{ tr('Creator') }}</td>
                                    <th scope="row">{{ Str::ucfirst($pages->users->name) }}</th>
                                </tr>
                                <tr>
                                    <td>{{ tr('Modifier') }}</td>
                                    <th scope="row">
                                        @if ($pages->modifier_id)
                                            {{ Str::ucfirst($pages->modifiers->name) }}
                                        @else
                                ...
                                <tr>
                                    <td>{{ tr('Created At') }}</td>
                                    <th scope="row">
                                        {{ $pages->created_at->format('d.m.Y') }} <br/>
                                        {{ $pages->created_at->format('H:i') }}
                                    </th>
                                </tr>
                                <tr>
                                    <td>{{ tr('Updated At') }}</td>
                                    <th scope="row">
                                        {{ $pages->updated_at->format('d.m.Y') }} <br/>
                                        {{ $pages->updated_at->format('H:i') }}
                                    </th>
                                </tr>
                                </tbody>
                            </table>
