<div class="card card-primary">
    <div class="card-header">
        <h3 class="card-title">{{ tr('Links') }}</h3>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-12 my-2 d-flex justify-content-between">
                <a class="btn btn-primary create-btn" href="{{ route('links.create') }}">
        ...
        <div class="table-responsive">
            <table class="table table-hover table-wrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ Illuminate\Support\Str::of($link->title)->words(4) }}</td>
                        <td>{{ $link->slug }}</td>
                        <td>{!! Illuminate\Support\Str::of($link->description)->words(6) !!}</td>
                        <td>{{ $link->count_view }}</td>
                        <td>
                            @if ($link->status == 2)
                        ...
                        <td>{{ Str::ucfirst($link->users->name) }}</td>
                        <td class="d-flex">
                            <a class="btn btn-primary m-1" href="{{ route('links.show', $link->id) }}" title="View"
                                aria-label="View"><span class="fas fa-eye"></span>
                            </a>
                            <a class="btn btn-primary m-1" href="{{ route('links.edit', $link->id) }}" title="Янгилаш"
            ...
            <span class="d-flex pt-2 justify-content-end"> {{ $links->links() }}</span>
        </div>
    </div>
</div>
